<template>
    <el-card>
        ...
        <el-form :inline="true" style="text-align:right;">
            <el-form-item>
                <el-button @click="addData" type="primary"><i class="el-icon-plus"></i> Tambah Slider</el-button>
            </el-form-item>
            <el-form-item>
        ...
        @sort-change="sortChange">
            ...
            :filters="[{value: 0, text: 'Tidak'},{value: 1, text: 'Ya'}]">
                <template slot-scope="scope">
                    <span :class="scope.row.status ? 'text-success' : 'text-danger'">{{scope.row.status ? 'Ya' : 'Tidak'}}</span>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        store: function() {
            this.loading = true;
            axios.post(BASE_URL + '/carousel', this.formModel)
                .then(r => {
                    this.loading = false;
                    this.showForm = false;
                    this.$message({
        ...
        update: function() {
            this.loading = true;
            axios.put(BASE_URL + '/carousel/' + this.formModel.id, this.formModel)
                .then(r => {
                    this.loading = false;
                    this.showForm = false
                    this.$message({
        ...
        deleteData: function(id) {
            this.$confirm('Anda yakin akan menghapus data ini?')
                .then(() => {
                    axios.delete(BASE_URL + '/carousel/' + id)
                        .then(r => {
                            this.requestData();
                            this.$message({
        ...
        requestData: function() {
            ...
            axios.get(BASE_URL + '/carousel', {params: Object.assign(params, this.filters)})
                .then(r => {
                    this.loading = false;
